  ...
  return (
    <div className="space-y-16 py-8 sm:py-12">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center rounded-3xl bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white p-8 sm:p-12 shadow-2xl relative overflow-hidden">
          ...
          <div className="lg:col-span-7 space-y-6 relative z-10">
            <div className="inline-flex items-center gap-2 rounded-full bg-teal-500/20 border border-teal-500/30 px-3.5 py-1 text-xs font-semibold text-teal-300">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {['Agribusiness', 'Banking & Finance', 'Public Policy', 'Corporate Governance', 'Marketing'].map((tag, i) => (
                <span
                  key={i}
                  className="rounded-lg bg-slate-800/80 px-2.5 py-1 text-xs font-medium text-slate-300 border border-slate-700/80"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="lg:col-span-5 relative z-10">
            <div className="rounded-2xl border border-slate-700/80 bg-slate-800/80 p-6 backdrop-blur-md shadow-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-700/60 pb-3">
                <span className="rounded-md bg-teal-500/20 text-teal-300 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider">
                  Featured Issue
                </span>
              ...
              <div className="rounded-xl bg-slate-900/90 p-3.5 border border-slate-800 space-y-2 text-xs">
                <div className="flex items-center justify-between text-slate-400">
                  <span>Peer-Reviewed Articles:</span>
                  <span className="font-bold text-white">{articles.filter((a) => a.issueId === currentIssue.id).length} Papers</span>
                </div>
                <div className="flex items-center justify-between text-slate-400">
                  <span>Publication Date:</span>
                  <span className="font-semibold text-teal-300">{currentIssue.publicationDate}</span>
                </div>
              </div>
              ...
              <div className="pt-2 flex items-center justify-between gap-2">
                <button
                  onClick={() => onNavigate('current-issue')}
                  className="w-full inline-flex items-center justify-center gap-1.5 rounded-xl bg-teal-600 py-2.5 text-xs font-bold text-white hover:bg-teal-500 transition shadow"
                >
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            ...
            <h2 className="font-serif text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 mt-0.5">
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="rounded-md bg-teal-50 px-2.5 py-1 font-semibold text-teal-800 border border-teal-200/60">
                    {art.category}
                  </span>
                ...
                <h3
                  ...
                  className="cursor-pointer font-serif text-lg font-bold text-slate-900 hover:text-teal-700 transition leading-snug"
                >
                  {art.title}
                </h3>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-slate-900 text-white p-8 sm:p-10 shadow-xl space-y-8">
              ...
              >
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    <span className="rounded bg-teal-500/20 text-teal-300 px-2 py-0.5 font-bold text-[10px] uppercase">
                      {evt.eventType}
                    </span>
                    <span className="font-mono text-[11px] text-slate-400">{evt.eventDate}</span>
                  </div>
      ...
      {nextCall && (
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl border border-teal-200 bg-gradient-to-r from-teal-50 via-blue-50 to-teal-50 p-8 sm:p-10 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-6">
            <div className="space-y-2 max-w-2xl">
              <span className="inline-block rounded-full bg-teal-600 px-3 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">
              ...
              <div className="flex flex-wrap gap-4 pt-2 text-xs font-semibold text-slate-700">
                <span>Submission Deadline: <strong className="text-teal-900">{nextCall.submissionDeadline}</strong></span>
                <span>•</span>
                <span>Expected Release: <strong>{nextCall.expectedPublicationDate}</strong></span>
              </div>
            </div>
