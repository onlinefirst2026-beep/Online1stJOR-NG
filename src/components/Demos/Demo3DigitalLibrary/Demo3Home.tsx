...
export const Demo3Home: React.FC<Demo3HomeProps> = ({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:py-12 space-y-12">
      ...
      <section className="rounded-2xl border border-stone-300 bg-white p-6 sm:p-8 shadow-xs font-sans space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-stone-300 pl-10 pr-4 py-3 text-xs sm:text-sm text-stone-900 focus:border-amber-700 focus:outline-none bg-stone-50/50"
          />
        </div>

        <div className="flex flex-wrap items-center gap-2 pt-1 text-xs">
          <span className="font-semibold text-stone-500">Filter by section:</span>
          {disciplines.map((d) => (
            <button
              key={d}
      ...
      <section className="rounded-2xl border border-stone-300 bg-white p-8 shadow-xs space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-stone-200 pb-4 font-sans">
          <div>
        ...
        {currentIssue.description && (
          <p className="text-xs sm:text-sm text-stone-600 leading-relaxed italic border-l-2 border-amber-800 pl-4">
            "{currentIssue.description}"
          </p>
        )}
          ...
          {filteredArticles.map((art) => (
            <article key={art.id} className="py-6 space-y-3 first:pt-2 last:pb-0">
              ...
              <h3
                ...
                className="cursor-pointer font-serif text-xl sm:text-2xl font-bold text-[#1c1917] hover:text-amber-900 transition leading-snug"
              >
                {art.title}
              </h3>

              <p className="font-sans text-xs font-semibold text-stone-800">
                {art.authors.map((a) => a.name).join(', ')}
              </p>

              <p className="font-sans text-xs text-stone-600 leading-relaxed text-justify line-clamp-3">
                {art.abstract}
              </p>

              <div className="font-sans pt-2 flex flex-wrap items-center justify-between gap-2 text-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="rounded bg-[#1c1917] px-3 py-1 font-semibold text-white hover:bg-stone-800"
                  >
                    Download PDF
                  </button>
                </div>
      ...
      <section className="rounded-2xl border border-stone-300 bg-stone-100 p-6 sm:p-8 font-sans space-y-4">
